...
public class SkyblockMenus {
    ...
    // --- PROFILE / STATS MENU (The Detailed One) ---
    public static void openStatsMenu(PlayerEntity player) {
        ...
        NamedScreenHandlerFactory factory = new NamedScreenHandlerFactory() {
            ...
            public ScreenHandler createMenu(int syncId, PlayerInventory inv, PlayerEntity p) {
                ...
                gui.setStack(10, getArmorCopy(player, 3, "Helmet")); // Helmet
                gui.setStack(19, getArmorCopy(player, 2, "Chestplate")); // Chest
                gui.setStack(28, getArmorCopy(player, 1, "Leggings")); // Legs
                gui.setStack(37, getArmorCopy(player, 0, "Boots")); // Boots
                ...
                // --- Stats Categories (Right Side - Based on Video) ---
                
                // 1. Health (Apple/Heart)
                gui.setStack(15, createStatItem(Items.GOLDEN_APPLE, "§cHealth", player, StatType.HEALTH, 
                    "§7Your maximum health."));

                // 2. Defense (Iron Chestplate)
                gui.setStack(16, createStatItem(Items.IRON_CHESTPLATE, "§aDefense", player, StatType.DEFENSE, 
                    "§7Reduces damage taken."));

                // 3. Strength (Blaze Powder)
                gui.setStack(24, createStatItem(Items.BLAZE_POWDER, "§cStrength", player, StatType.STRENGTH, 
                    "§7Increases raw damage."));

                // 4. Intelligence (Enchanted Bottle)
                gui.setStack(25, createStatItem(Items.EXPERIENCE_BOTTLE, "§bIntelligence", player, StatType.MANA, 
                    "§7Increases max mana."));

                // 5. Crit Chance (Skeleton Skull)
                gui.setStack(33, createStatItem(Items.SKELETON_SKULL, "§9Crit Chance", player, StatType.CRIT_CHANCE, 
                    "§7Chance to deal double damage."));

                // 6. Crit Damage (Wither Skull)
                gui.setStack(34, createStatItem(Items.WITHER_SKELETON_SKULL, "§9Crit Damage", player, StatType.CRIT_DAMAGE, 
                    "§7Multiplier on critical hits."));
    ...
    public static void handleButtonClick(PlayerEntity player, ItemStack clickedItem) {
        ...
        if (name.contains("Close")) {
            ...
            // Logic to switch menus
            if (name.contains("Profile")) {
                openStatsMenu(player); // Switch to Stats
            } else {
                openMainMenu(player); // Switch Back to Main
            }
        }
    }
