...
public class Hyperion extends SBItem {
    ...
    public Map<StatType, Double> getStats() {
        ...
        // Exact Clean Hyperion Stats
        stats.put(StatType.GEAR_SCORE, 673.0);
        stats.put(StatType.DAMAGE, 293.0);
        stats.put(StatType.STRENGTH, 150.0);
        stats.put(StatType.CRIT_CHANCE, 10.0);
        stats.put(StatType.CRIT_DAMAGE, 50.0); // Hidden base stat usually
        stats.put(StatType.HEALTH, 20.0);
        stats.put(StatType.DEFENSE, 20.0);
    ...
    public Ability getAbility() {
        return new Ability("Wither Impact", 
            ...
            public void onActivate(PlayerEntity player, World world) {
                ...
                if (hit.getType() != HitResult.Type.MISS) dest = dest.subtract(direction.multiply(0.5));
                ...
                // Damage & Shield
                double radius = 6.0;
                List<Entity> targets = world.getOtherEntities(player, new Box(dest.x - radius, dest.y - radius, dest.z - radius, dest.x + radius, dest.y + radius, dest.z + radius));
                
                ProfileManager pm = ProfileManager.getServerInstance((ServerWorld) world);
                SkyblockProfile profile = pm.getProfile(player);
                float magicDmg = DamageCalculator.calculateMagicDamage(player, profile, 10000.0, 0.3);
